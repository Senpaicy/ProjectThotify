...
module.exports = {
    checkId(id, variableName) {
        ...
        if (id.length === 0) throw `Error: ${variableName} cannot be an empty string.`;
        if (!ObjectId.isValid(id)) `Error: ${variableName} is an invalid object id.`;
    ...
    checkString(stringVal, variableName, allowNumbers) {
        ...
        if (!(/^[a-zA-Z]+$/.test(stringVal))) throw `Error: ${stringVal} should only contain letters.`;
        return stringVal;
    },
