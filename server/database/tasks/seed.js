...
async function main() {
  ...
  const eUser = {
    ...
    bio: {
      description:
        "i hope eric works at spotify one day due to his amazing spotify api skills.",
      funFact: "ahaha",
      other: "i like that",
    },
  };
  ...
  const cindy = await users.addUser(
    cUser.firstName,
    cUser.lastName,
    cUser.email,
    cUser.bio
  ...
  const jacob = await users.addUser(
    jUser.firstName,
    jUser.lastName,
    jUser.email,
    jUser.bio
  ...
  const eric = await users.addUser(
    eUser.firstName,
    eUser.lastName,
    eUser.email,
    eUser.bio
  ...
  const farhan = await users.addUser(
    fUser.firstName,
    fUser.lastName,
    fUser.email,
    fUser.bio
  ...
  console.log(
    "------- Attempting to get cindy's user data from user database..."
  );
  ...
  console.log(
    "------- Attempting to get jacob's user data from user database..."
  );
  ...
  console.log(
    "------- Attempting to remove farhan's user data from user database..."
  );
  const f_delete_info = await users.removeUser(farhan_id);
  console.log(
    "------- Finished removing farhan's user data from user database."
  );
  ...
  const cUserUpdate = {
    pfp_url:
      "https://media.licdn.com/dms/image/C4E03AQFWIfoJD2ddMA/profile-displayphoto-shrink_800_800/0/1613668127608?e=1677110400&v=beta&t=X7_-UGAQiiiJjYnES4Hn6St5tn_5lxjahCYRCSjB9R8",
    firstName: "Cindy",
    lastName: "Zhang",
  ...
  const jUserUpdate = {
    ...
    spotifyUsername: "",
    pfp_url:
      "https://media.licdn.com/dms/image/C4E03AQE1oNBNmefEpA/profile-displayphoto-shrink_200_200/0/1660327397872?e=1677110400&v=beta&t=cfaI79iDBU__0PBYJ_cqjxX4gXeiukAxEx_g6X4-A9w",
    bio: {
      description: "jacob is a god tier developer n kill the chat!",
      funFact: "i need a joke from jacob.",
      other: "but he's probably asleep.",
  ...
  const eUserUpdate = {
    ...
    spotifyUsername: "",
    pfp_url:
      "https://media.licdn.com/dms/image/C4D03AQFD1-yRKU941A/profile-displayphoto-shrink_100_100/0/1636663159941?e=1677110400&v=beta&t=Rg2_XrY6cToD75Pm0ERiK-OAB-sCjA70SuR4fN42j4k",
    bio: {
      description:
        "i hope eric works at spotify one day due to his amazing spotify api skills.",
      funFact: "ahaha",
  ...
  console.log(
    "------- Attempting to get Users from a Chat in the chat database: "
  );
  ...
  const cindys_message = {
    ...
    timestamp:
      cindys_message_time.toDateString() +
      " " +
      cindys_message_time.toTimeString(),
  };
  ...
  const addedCMessage = await chats.addMessageToChat(
    tChat.chatName,
    cindys_message
  );
